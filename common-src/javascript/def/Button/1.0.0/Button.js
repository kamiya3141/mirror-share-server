...
class TSOButton extends TSObject {
	...
	after_init() {
		...
		this.addEventListener("mouseover", e => {
			if (this.disabled) return;
			e.stopPropagation();
			TSObject.mouseHover(this, 0.5);
			this.#mouseOverHandler();
		});
		this.addEventListener("mouseleave", e => {
			if (this.disabled) return;
			e.stopPropagation();
			TSObject.mouseLeave(this);
	...
	set baseBlur(tf = false) {
		tf = Boolean(tf);
		this.#baseBlur = tf;
		if (!this.#baseBlur != tf && tf == true) this.#myOutline.latest = this.#myOutline.default;
	}
	get baseBlur() {
