...
class TSOMessage extends TSObject {
	...
	init() {
		if (!TSOMessage.cachedTemplate) {
			const file_name = window.getMyTemplateFileUrl(TSOMessage.mySrc);
			fetch(file_name).then(res => res.text()).then(templateText => {
				TSOMessage.cachedTemplate = new DOMParser().parseFromString(templateText, "text/html");
				this.after_init(TSOMessage.myElementName);
			});
		} else this.after_init(TSOMessage.myElementName);
	}
	after_init(_element_name) {
		this.shadowRoot.prepend(TSOMessage.cachedTemplate.getElementById("template").content.cloneNode(true));
		const my_event = new CustomEvent(TSOMessage.myEventName);
		this.appendChild(new DOMParser().parseFromString(`<div id="${this.deleteDivId}" style="display:inline-block;font-size:inherit;border-left:${this.stroke} ${this.strokeWeight} ${this.strokeType};">&#9003;</div>`, "text/html").getElementById(this.deleteDivId)).addEventListener("click", e => {
			if (e.button == 0) {
				this.remove();
		...
		this.shapeValue = ["50%", "auto", "auto", "auto"].join(",");
	...
	completeSetEnd() {
		this.shapeValue = `50%,${this.getBoundingClientRect().y},auto,auto`;
		this.radius = "1rem";
