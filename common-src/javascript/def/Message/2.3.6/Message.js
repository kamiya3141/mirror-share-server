...
class TSOMessage extends TSObject {
	...
	constructor(msg, msg_type) {
		super();
		this.#inputFirstMsgFlag = Boolean(msg);
		if (this.#inputFirstMsgFlag) this.innerHTML += msg;
	...
	init() {
		if (!window.TSObjectTemplateMap.has(TSOMessage.cachedFileName)) {
			const file_name = window.TSObjectTemplateFileUrl(TSOMessage.cachedFileName);
	...
	after_init() {
		...
		this.appendChild(new DOMParser().parseFromString(`<div id="${this.deleteDivId}" style="display:inline-block;font-size:inherit;border-left:${this.border};">&times;</div>`, "text/html").getElementById(this.deleteDivId)).addEventListener("click", e => {
			e.stopPropagation();
			if (e.button == 0) {
				this.remove();
		...
		mySlot.addEventListener("slotchange", () => {
			...
			[...this.children].forEach(c => {
				...
				window.TSObjectMouseAddEventListener(c, 1);
				c.addEventListener("click", e => {
					e.stopPropagation();
					if (c.id != this.deleteDivId) {
						const _href = c.getAttribute("href");
		...
		this.shapeValue = `50%,auto,${TSOMessage.usingMobileBoolean ? TSOMessage.usingMobileValue : "auto"},auto,1rem`;
		...
		this.style.opacity = ((TSOMessage.loaded || this.#inputFirstMsgFlag) ? "1" : "0");
	...
	static set usingMobileBoolean(_tf) {
		_tf = Boolean(_tf);
		TSOMessage.usingMobile.tf = _tf;
		if (!_tf) {
			TSOMessage.usingMobile.val = "auto";
		}
	}
	static get usingMobileBoolean() {
		return TSOMessage.usingMobile.tf;
	}
	static set usingMobileValue(_vl) {
		if (!TSOMessage.usingMobileBoolean) return;
		TSOMessage.usingMobile.val = _vl;
	}
	static get usingMobileValue() {
		return TSOMessage.usingMobile.val
	}

	completeSetBegin() {
		const _spvl = this.shapeValue;
		this.#saveMyWH = [TSOMessage.usingMobileBoolean ? TSOMessage.usingMobileValue : "auto", _spvl[3]];
