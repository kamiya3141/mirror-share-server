...
class TSOMessage extends TSObject {
	...
	after_init() {
		...
		this.appendChild(new DOMParser().parseFromString(`<div id="${this.deleteDivId}" style="display:inline-block;font-size:inherit;border-left:${this.getBorder()};">&times;</div>`, "text/html").getElementById(this.deleteDivId)).addEventListener("click", e => {
			if (e.button == 0) {
				this.remove();
