...
class TSObject extends HTMLElement {
	...
	#myAttributes = [["st", "rect", arg => this.shapeType = arg], ["sv", "0,0,100%,100%", arg => this.shapeValue = arg], ["fill", "var(--tso-bc-cl)", arg => this.fill = arg], ["stroke", "var(--tso-bd-cl)", arg => this.stroke = arg], ["stroke-weight", "var(--tso-ln-wei)", arg => this.strokeWeight = arg], ["position-type", "lt", arg => this.positionType = arg], ["allow-hover", this.#allowHoverValue, arg => this.allowHover = arg], ["tv", "", arg => this.text = arg], ["tt", "b", arg => this.textType = arg], ["tpv", "50%,50%,50%,50%", arg => this.textPositionValue = arg], ["tpt", "cxcy", arg => this.textPositionType = arg], ["tc", "var(--tso-black)", arg => this.textColor = arg]];
	constructor() {
		super();
	...
	set textColor(col = "var(--tso-black)") {
		this.textElement.style.color = col;
	}
	get textColor() {
	...
	set textType(val = "b") {
		if (!/b|i|ib|n/.test(val)) console.error(`textTypeに無効な値が挿入されました。"${val}"`);
			...
			this.textElement.setAttribute("tt", val);
