...
// 長くてごめん
window.getCurrentScriptUrl = (with_file_name, _my_src) => {
	let return_url;
	const _src = String(Boolean(_my_src) ? _my_src : document.currentScript.src);
	if (!_src.includes("?")) {
		if (new URL(_src).hostname.split(".")[0] == "share") return_url = _src;
		else return_url = _src.split("/").slice(0, _src.split("/").length - Number(!Boolean(with_file_name))).join("/");
	} else {
...
class TSObject extends HTMLElement {
	...
	#myAttributes = [["st", "rect", arg => this.shapeType = arg], ["sv", "0,0,100%,100%", arg => this.shapeValue = arg], ["fill", "transparent", arg => this.fill = arg], ["stroke", "transparent", arg => this.stroke = arg + TSObject.addRenewOptionString], ["stroke-weight", "var(--tso-bd-wei)", arg => this.strokeWeight = arg + TSObject.addRenewOptionString], ["stroke-type", "var(--tso-bd-tp)", arg => this.strokeType = arg + TSObject.addRenewOptionString], ["position-type", "lt", arg => this.positionType = arg], ["allow-hover", "false", arg => this.allowHover = arg], ["tv", "", arg => this.text = arg], ["tt", "b", arg => this.textType = arg], ["tpv", "50%,50%,50%,50%", arg => this.textPositionValue = arg], ["tpt", "cxcy", arg => this.textPositionType = arg], ["tc", "transparent", arg => this.textColor = arg], ["ts", "1rem", arg => this.textSize = arg], ["allow-hover", "false", arg => this.allowHover = arg]];
	local_chached = false;
	static addRenewOptionString = "-:-";
	...
	init_org() {
		if (!window.TSObjectTemplateMap.has(TSObject.cachedFileName)) {
			const file_name = window.TSObjectTemplateFileUrl(TSObject.cachedFileName);
	...
	after_init_org(_tf) {
		...
		this.#myAttributes.forEach(c => {
			if (!Boolean(this.hasAttribute(c[0]))) this.setAttribute(c[0], c[1]);
	...
	set fill(col = "var(--tso-bc-cl)") {
		if (col == null || col == "none") col = "rgba(0, 0, 0, 0)";
		this.setAttribute("fill", col.includes("!important") ? col.replace("!important", "") : col);
	...
	set border(arg) {
		if (!Boolean(arg)) arg = `${this.stroke} ${this.strokeWeight} ${this.strokeType}`;
		this.style.border = arg;
	}
	get border() {
		return `${window.getComputedStyle(this).borderColor} ${window.getComputedStyle(this).borderStyle} ${window.getComputedStyle(this).borderWidth}`;
	}

	// ふちの色
	set stroke(arg = "var(--tso-bd-cl)") {
		if (!Boolean(arg) || arg == "none") arg = "var(--clear)" + TSObject.addRenewOptionString;
		arg = String(arg);
		let _tf = true;
		if (arg.includes(TSObject.addRenewOptionString)) {
			_tf = !_tf;
			arg = arg.replace(TSObject.addRenewOptionString, "");
		}
		this.setAttribute("stroke", arg);
		this.style.borderColor = arg;
		if (_tf) this.border = null;
	}
	get stroke() {
		return window.getComputedStyle(this).borderColor;
	}
	set strokeWeight(arg = "var(--tso-bd-wei)") {
		if (!Boolean(arg)) arg = "var(--tso-bd-wei)" + TSObject.addRenewOptionString;
		arg = String(arg);
		let _tf = true;
		if (arg.includes(TSObject.addRenewOptionString)) {
			_tf = !_tf;
		...
		arg = this.arrangedValue(arg);
		this.setAttribute("stroke-weight", arg);
		this.style.borderWidth = arg;
		if (_tf) this.border = null;
	}
	get strokeWeight() {
		return this.onlyNumber(window.getComputedStyle(this).borderWidth);
	}
	set strokeType(arg = "var(--tso-bd-tp)") {
		if (!Boolean(arg)) arg = "var(--tso-bd-tp)" + TSObject.addRenewOptionString;
		arg = String(arg);
		let _tf = true;
		if (arg.includes(TSObject.addRenewOptionString)) {
			_tf = !_tf;
...
window.TSObjectTemplateFileUrl = lib_name => `https://share.tshuto.com/common-src/javascript/def/${lib_name}/.${window.TSObjectTemplateFileExt}`;
