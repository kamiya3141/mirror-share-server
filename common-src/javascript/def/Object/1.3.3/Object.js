...
window.getMyLowerFileName = with_ext => String(window.getMyFileName(with_ext)).toLowerCase();
...
class TSObject extends HTMLElement {
	...
	#myAttributes = [["st", "rect", arg => this.shapeType = arg], ["stroke-weight", "var(--tso-bd-wei)", arg => this.strokeWeight = arg], ["sv", "0,0,100%,100%", arg => this.shapeValue = arg], ["fill", "var(--tso-bc-cl)", arg => this.fill = arg], ["stroke", "var(--tso-bd-cl)", arg => this.stroke = arg], ["stroke-type", "var(--tso-bd-tp)", arg => this.strokeType = arg], ["position-type", "lt", arg => this.positionType = arg], ["allow-hover", this.#allowHoverValue, arg => this.allowHover = arg], ["tv", "", arg => this.text = arg], ["tt", "b", arg => this.textType = arg], ["tpv", "50%,50%,50%,50%", arg => this.textPositionValue = arg], ["tpt", "cxcy", arg => this.textPositionType = arg], ["tc", "var(--tso-black)", arg => this.textColor = arg], ["ts", "1rem", arg => this.textSize = arg], ["allow-hover", "false", arg => this.allowHover = arg]];
	static cachedTemplate = null;
	...
	init_org() {
		if (!TSObject.cachedTemplate) {
			const file_name = window.getMyTemplateFileUrl(TSObject.mySrc);
			fetch(file_name).then(res => res.text()).then(templateText => {
				TSObject.cachedTemplate = new DOMParser().parseFromString(templateText, "text/html");
				this.after_init_org();
			});
		} else this.after_init_org();
	}
	after_init_org() {
		this.shadowRoot.appendChild(TSObject.cachedTemplate.getElementById("template").content.cloneNode(true));
	...
	#border() {
		this.style.border = `${this.stroke} ${this.strokeWeight} ${this.strokeType}`;
	}
	...
	get strokeWeight() {
		return this.arrangedValue(window.getComputedStyle(this).borderWidth);
	}
