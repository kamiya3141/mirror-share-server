...
class TSObject extends HTMLElement {
	...
	#myAttributes = [["st", "rect", arg => this.shapeType = arg], ["sv", "0,0,100%,100%", arg => this.shapeValue = arg], ["fill", "var(--bc-cl)", arg => this.fill = arg], ["stroke", "var(--bd-cl)", arg => this.stroke = arg], ["stroke-weight", "var(--ln-wei)", arg => this.strokeWeight = arg], ["position-type", "lt", arg => this.positionType = arg], ["tv", "", arg => this.text = arg], ["allow-hover", this.#allowHoverValue, arg => this.allowHover = arg]];
	constructor() {
		super();
		this.attachShadow({ mode: "open" });
		this.shadowTemplateURL = new URL(`./.${window.TSObjectTemplateFileExt}`, new URL(document.querySelector("script[src*='Object.js']").getAttribute("src")));
		(async () => {
			const res = await fetch(this.shadowTemplateURL);
			...
			this.innerHTML += `<slot name="${this.#textSlotName}"></slot>`;
	...
	set text(val = "") {
		this.querySelector(`slot[name="${this.#textSlotName}"]`).innerHTML = val;
	}
	get text() {
		return this.querySelector(`slot[name="${this.#textSlotName}"]`).innerHTML;
	}
	// 座標決定時の要素の相対位置
