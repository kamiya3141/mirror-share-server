...
PROMPT_COLOR_USER="\[\033[94m\]"
PROMPT_COLOR_HOST="\[\033[96m\]"
PROMPT_COLOR_SYMBOL="\[\033[34m\]"
PROMPT_COLOR_CDIR="\[\033[32m\]"
...
function set_prompt_command() {
	...
	local correct_str="${PROMPT_COLOR_CORRECT}C${PROMPT_COLOR_RESET}"
	local failed_str="${PROMPT_COLOR_FAILED}F${PROMPT_COLOR_RESET}"
	...
	local cdir_str="${PROMPT_COLOR_CDIR}$(pwd)${PROMPT_COLOR_RESET}"
	local main_str="${PROMPT_COLOR_MAIN}${PROMPT_BG_COLOR_MAIN}┌${PROMPT_COLOR_RESET}${PROMPT_BG_COLOR_RESET}"

	main_str+=$(surround_item_char "${result_str}" "${current_date} ${current_time}" "${user_str}" "${host_str}" "${cdir_str}")
	main_str+="\n${PROMPT_COLOR_MAIN}${PROMPT_BG_COLOR_MAIN}└${PROMPT_COLOR_RESET}${PROMPT_BG_COLOR_RESET}\$"
