...
switch ($target_query) {
	...
	default:
		...
		if (isset($redirectMap[$target_query])) {
			...
			} else if ($a == 'blog' && !preg_match('/^src*/i', $other_data_query) && !str_contains($other_data_query, INDEX_HTML))
				$remoteUrl = url_join(getMyHostName($a), INDEX_HTML);
			else
				$remoteUrl = url_join($remoteUrl, $other_data_path);
			if ($convert_query_exist)
				echo $remoteUrl;
			else
				forwardRemoteFile($remoteUrl, false, (($a == LINK_STRING && !preg_match('/^src*/i', $other_data_query)) ? true : false));
			exit;
		} else
